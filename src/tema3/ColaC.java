...
public class ColaC
{
		...
		if (!this.Llena())
		{
			if (this.Vacia())
				...
				if (fin + 1 == cola.length)
					...
					fin++;
			cola[fin] = obd.Entero("Nuevo numero: ");
			System.out.println("Numero insertado con exito...");
		}
		else
	...
	public void Extraer()
	{
		if (!this.Vacia())
		{
			System.out.println("Numero extraido " + cola[ini]);
			if (ini == fin)
				ini = fin = -1;
			else
	...
	public void Recorrido()
	{
		if (!this.Vacia())
		{
			...
			/*
			 * if (fin >= ini) for (tam = ini; tam <= fin; tam++)
			 * System.out.println(cola[tam]); else {/* /* for (tam = ini; tam < cola.length;
			 * tam++) System.out.println(cola[tam]); for (tam = 0; tam <= fin; tam++)
